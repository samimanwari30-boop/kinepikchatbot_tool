...
def run_ksea_top_visualisation(perturbation="AZD3759", cell_line="MCF7", direction=None):
    ...
    # Mirrors plot_ksea's own sort/label logic exactly, so the text below can
    # never drift out of sync with what the chart actually shows.
...
def run_phosphosite_viewer(protein):
    """Show a paginated, searchable table of the phosphosites a kinase targets."""
    protein_id, error = _normalise_or_error(protein)
    if error:
        return error

    all_result = get_kinase(protein_id, phosphosites="targets")

    table_path = build_phosphosite_table(protein, protein_id, all_result)
    if not table_path:
        return {
            "reply": _with_source(format_phosphosite_viewer(protein, protein_id, all_result), "kinases/specific"),
            "image": None,
        }

    return {
        "reply": _with_source(
            f"Phosphosites targeted by {protein} ({protein_id}) — browse the table below.",
            "kinases/specific",
        ),
        "image": None,
        "table": table_path,
...
def run_network_visualisation(protein):
    ...
    image_path = plot_network(records, protein, kinase_id=protein_id)
