...
def plot_network(records, central_label, kinase_id=None, kinase_only=True):
    ...
    kinase_only restricts nodes to genuine kinases (cross-referenced against
    KINEPIK's own kinase list) when True; when False, every connected protein
    in the raw interaction data is kept, including non-kinase substrates.
    """
    try:
    ...
    from utils import batch_uniprot_to_gene, looks_like_uniprot_id, _is_error_payload
    from kinepik_api import get_all_kinases
    ...
    # Restrict the network to kinase-to-kinase edges only, if requested. A
    # kinase's raw interaction data includes non-kinase substrates
    # (transcription factors, structural proteins, etc.) — cross-reference
    # every node against KINEPIK's own kinase list so only genuine kinases
    # remain, unless the caller explicitly asked to include everything.
    if kinase_only:
        kinase_result = get_all_kinases()
        if not _is_error_payload(kinase_result):
            kinase_ids = set()
            for item in kinase_result or []:
                entry = item if isinstance(item, dict) else (item[0] if isinstance(item, list) and item else None)
                if isinstance(entry, dict) and entry.get("UniprotID"):
                    kinase_ids.add(entry["UniprotID"])
            if kinase_ids:
                central_ref = kinase_id if kinase_id else central_label
                non_kinase_nodes = [n for n in G.nodes if n != central_ref and n not in kinase_ids]
                G.remove_nodes_from(non_kinase_nodes)
    ...
    title_scope = "kinases" if kinase_only else "proteins"
    ax.set_title(
        f"Interaction Network — {central_name}\n"
        f"Node size = network connectivity · Edges shown between all connected {title_scope}",
