...
def _extract_sites(result):
    ...
    for item in result:
        if not isinstance(item, dict):
            continue
        raw = item.get("TargetPhosphosites", item.get("target_phosphosites", []))
        if isinstance(raw, list):
            sites.extend(str(s) for s in raw if s)
    return list(dict.fromkeys(sites))


def build_phosphosite_table(protein_name, protein_id, all_result):
    """Build a paginated HTML table of phosphosites a kinase targets.
...
<div class="table-wrap">
    <div class="table-header">
        <h3>Phosphosites — {safe_name} ({safe_id})</h3>
...
<script>
    ...
    abs_path = _save_static("phosphosite_table.html")
    with open(abs_path, "w") as f:
        f.write(page_html)

    return "static/phosphosite_table.html"
