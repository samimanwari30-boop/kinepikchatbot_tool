...
def format_phosphosite_viewer(protein_name, protein_id, all_result, limit=50, mode="targets"):
    ...
    mode="own" reads the kinase's own autophosphorylation sites instead of
    the sites it targets on other proteins."""
    ...
    key, alt_key = ("PhosphositesOnKinase", "phosphosites_on_kinase") if mode == "own" else ("TargetPhosphosites", "target_phosphosites")
    ...
    try:
        ...
        for item in all_result:
            if not isinstance(item, dict):
                continue
            sites = item.get(key, item.get(alt_key, []))
            if isinstance(sites, list):
                all_sites.extend(str(s) for s in sites if s)
        all_sites = list(dict.fromkeys(all_sites))

        if not all_sites:
            noun = "own phosphosites" if mode == "own" else "phosphosite targets"
            return f"No {noun} found for {protein_name}."
        ...
        heading = "Phosphosites on" if mode == "own" else "Phosphosites targeted by"
        return (
            f"{heading} {protein_name} ({protein_id}):\n\n"
            f"{table}"
            f"{footnote}"
        )
    except Exception:
