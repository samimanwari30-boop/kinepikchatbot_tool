...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Show a full, paginated, searchable table of every phosphosite a kinase targets. "
                "Use when the user wants to browse or search the complete list of a kinase's "
                "targets, e.g. 'show me the phosphosites for CDK1', 'list all targets of EGFR'. "
                "Do NOT use this if the user names a specific substrate to check — use "
                "kinase_substrate_check instead."
            ),
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Show a network diagram of how a kinase connects to other kinases. "
                "Use when the user asks how a kinase fits in, its network, relationships, "
                "connections, or signalling map."
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
def _dispatch_tool(tool_name, args, session_id=None):
    ...
    if tool_name == "phosphosite_viewer":
        return run_phosphosite_viewer(args["protein"])
    ...
    if tool_name == "network_visualisation":
        return run_network_visualisation(args["protein"])
